import os
import game
from gibo import read_gibo
from gibo import init_board_from_gibo
...
def board2net(board):
    ...
    #내 움직임
    my_moves, my_takes = game.get_all_moves(board)

    #상대방 움직임
    yo_board = game.rot_board(board)
    yo_moves, yo_takes = game.get_all_moves(board)
    yo_moves = game.rot_moves(yo_moves)
    yo_takes = game.rot_moves(yo_takes)

    moves = my_moves + yo_moves
    takes = my_takes + yo_takes

    net_board = np.zeros([10, 9, 42], np.float32)
    for y in range(10):
        for x in range(9):
            if board[y, x] != 0:
                net_board[y, x, board[y, x] - 1] = 1

    for move in moves:
        if move == game.MOVE_EMPTY:
            continue
        pos_from = move[0]
        pos_to = move[1]
        channel = 14 + board[pos_from] - 1
...
def move2net(move):
    net_move = np.zeros([10, 9, 2], np.float32)
    if move != game.MOVE_EMPTY:
        pos_from = move[0]
        pos_to = move[1]
